...
import { KeyTypesEnum } from '@typo3/backend/enum/key-types.js';
import '@typo3/backend/element/progress-bar-element.js';
import { ModifyFilesModal } from '@typo3/backend/uploader/modify-files-modal.js';
/**
...
class FilenameConflictModal {
    constructor(irreObjectUid, defaultAction, uploadHandler, questions) {
        this.irreObjectUid = irreObjectUid;
  ...
  ${this.questions[i].original.thumbUrl !== ''
      ...
      `;
        ...
        new RegularEvent('change', (event, target) => {
            if (target.value !== '') {
                ...
                for (const select of modal.querySelectorAll('.t3js-actions')) {
                    ...
                    this.questions[index].action = select.value;
                }
            }
            else {
        ...
        new RegularEvent('change', (event) => {
            ...
            this.questions[index].action = actionSelect.value;
...
class FileUploadHandler {
    constructor(middlewares, dragUploader) {
...
class CheckFileSize {
    ...
    checkFile(queueItem) {
        if (this.maxFileSize > 0 && queueItem.file.size > this.maxFileSize) {
            queueItem.markAsFailed(TYPO3.lang['file_upload.maxFileSizeExceeded']
                .replace(/\{0\}/g, queueItem.file.name)
                .replace(/\{1\}/g, DragUploader.fileSizeAsString(this.maxFileSize)));
        }
    }
}
class CheckFileDenyPattern {
    constructor(fileDenyPattern) {
...
class CheckAllowedDisallowedExtensions {
    constructor(filesExtensionsAllowed, filesExtensionsDisallowed) {
        this.filesExtensionsAllowed = filesExtensionsAllowed;
        this.filesExtensionsDisallowed = filesExtensionsDisallowed;
    }
    checkFile(queueItem) {
        const extension = queueItem.file.name.split('.').pop();
    ...
    checkAllowedExtensions(extension) {
    ...
    checkDisallowedExtensions(extension) {
...
class FileQueueItem {
    constructor(dragUploader, file, override) {
        this.state = QueueItemState.CHECKING;
        this.dragUploader = dragUploader;
        this.file = file;
    ...
    markAsFailed(error) {
        this.progressBar.finalize(SeverityEnum.error, error);
    }
    /**
