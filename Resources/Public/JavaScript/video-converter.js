...
export async function createMp4File (videoFile, onProgress) {
    ...
    params.push('-crf:v', '23', '-maxrate:v', '6M', '-bufsize:v', '12M'); // quality
...
export async function createHlsFiles (videoFile, onProgress, emitFile) {
    ...
    // params.push('-filter:v:0', 'scale=w=1280:h=720:force_original_aspect_ratio=decrease:force_divisible_by=2[720p]')

    // params.push('-map', '0:v');
    params.push('-map', '[720p]');
    params.push('-c:v:0', 'libx264'); // codec
    params.push('-crf:v:0', '23', '-maxrate:v:0', '6M', '-bufsize:v:0', '12M'); // quality
    ...
    params.push('-crf:v:1', '23', '-maxrate:v:1', '2M', '-bufsize:v:1', '4M'); // quality
    ...
    return new File([await ffmpeg.readFile('output.m3u8')], m3u8FileName, {type: 'application/x-mpegURL'});
}